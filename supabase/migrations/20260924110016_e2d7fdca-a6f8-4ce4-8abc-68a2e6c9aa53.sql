...
DECLARE
  v_col text;
BEGIN
  IF current_user NOT IN ('authenticated','anon') OR coalesce(auth.role(),'') = 'service_role' THEN
    RETURN NEW;
  END IF;
  IF public.is_active_staff(auth.uid(), ARRAY['super_admin','ops_manager']) THEN
    RETURN NEW;
  END IF;
